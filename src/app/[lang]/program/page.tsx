...
export default function ProgramPage({ params }: { params: { lang: string } }) {
  ...
  // ضع هنا رابط ملف البرنامج PDF
  const PROGRAM_PDF_URL =
  "https://drive.google.com/file/d/1D0PTHLC7dREktuPIRdfODSsmBADNhAZM/view?usp=sharing";
  ...
  return (
    <div>
      ...
      >
        ...
        >
          <h2 style={{ textAlign: "center", margin: 0, flex: 1 }}>{L.title}</h2>
                ...
                {filteredRows.map((r, i) => (
                  <tr key={i}>
                    ...
                    <td style={td}>
                      ...
                      {lang === "ar" ? (
                        r.titleEn ? (
                          <div style={{ opacity: 0.7, marginTop: 4 }}>{r.titleEn}</div>
                        ) : null
                      ) : r.titleAr ? (
                        <div style={{ opacity: 0.7, marginTop: 4 }}>{r.titleAr}</div>
                      ) : null}
                    </td>
